...
module.exports = {
    ...
    mitienda: async (sock, m, args, currentUser, config, reply, sender, groupId, userGroup) => {
        try {
            ...
            if (['abrir', 'cerrar', 'diseño', 'set-banner'].includes(subcomando)) {
                return reply(fmt.aviso('Comando obsoleto. Usa: `!mitienda diseñar [productos]`'));
            }
            ...
            if (subcomando === 'diseñar') {
                ...
                if (!diseño && !tieneMedia) {
                    return reply(fmt.aviso('Uso: !mitienda diseñar [productos] (Puedes adjuntar o responder a una foto o un GIF)'));
                }
                ...
                if (tieneMedia) {
                    // 1. Al iniciar el proceso, reacciona con un reloj de arena
                    await sock.sendMessage(groupId, { react: { text: '⏳', key: m.key } });
                    try {
                        ...
                        if (buffer) {
                            ...
                            if (urlSubida) {
                                ...
                                // 3. Si algo falla o se cae, marca la ❌
                                await sock.sendMessage(groupId, { react: { text: '❌', key: m.key } });
                ...
                // 2. Si todo fue exitoso, marca el ✅
                await sock.sendMessage(groupId, { react: { text: '✅', key: m.key } });
                return;
            }
    ...
    'mitienda añadir': async (sock, m, args, currentUser, config, reply, sender, groupId, userGroup) => {
        try {
            ...
            if (!nuevoProductoMatch) return reply(fmt.aviso('Formato inválido. Usa: [producto] - [precio]'));
